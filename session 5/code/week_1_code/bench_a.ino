...
void loop() {
  car.setSpeed(BOTH, 50);
  goDistance(508, false, true, 0);
  car.setSpeed(BOTH, 50);
  goDistance(510, false, true, 80);
  while (1);
  //  delay(300);
  //  car.setSpeed(BOTH, 0);
  //  delay(500);
...
void goDistance(int distance, bool correctD, bool correctA, int deadStop) {
  ...
  if (correctD) {
    ...
    if (getDistance(BOTH) + 2 > distance) {
      ...
      goDistance(correction, true, false, 20);
